...
struct CMDamageStatics
{
	...
	CMDamageStatics()
	{
		...
		// DEFINE_ATTRIBUTE_CAPTUREDEF(UCMCharacterAttributeSet, Armor, Target, false);

		// The Target's received Damage. This is the value of health that will be subtracted on the Target. We're not capturing this.
		DEFINE_ATTRIBUTE_CAPTUREDEF(UCMCharacterAttributeSet, Damage, Target, false);
	}
};

static const CMDamageStatics& DamageStatics()
{
	static CMDamageStatics DStatics;
	return DStatics;
}

UCMDamageExecCalculation::UCMDamageExecCalculation()
{
	// RelevantAttributesToCapture.Add(DamageStatics().ArmorDef);
	// We don't include Damage here because we're not capturing it. It is generated inside the ExecCalc.
}

void UCMDamageExecCalculation::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, OUT FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	// SetByCaller Damage
	float Damage = FMath::Max<float>(Spec.GetSetByCallerMagnitude(FGameplayTag::RequestGameplayTag(FName("Data.Damage")), true, -1.0f), 0.0f);
	...
	UCMAbilitySystemComponent* TargetASC = Cast<UCMAbilitySystemComponent>(TargetAbilitySystemComponent);
	if (TargetASC)
	{
		UCMAbilitySystemComponent* SourceASC = Cast<UCMAbilitySystemComponent>(SourceAbilitySystemComponent);
